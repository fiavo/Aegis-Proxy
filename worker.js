...
const PagesDafStati = 'https://aegis-panel.github.io/';
...
const NOVA_FREE_NOTICE = '🎁 ایجیس رایگان است، برای آن به کسی پول ندهید';
...
const NOVA_TG_CHANNEL = 'https://t.me/aegis_proxy';
...
const NOVA_TG_HANDLE = '@' + (String(NOVA_TG_CHANNEL).split('/').filter(Boolean).pop() || 'aegis_proxy');
...
const novaMachsanRaw = 'https://raw.githubusercontent.com/fiavo/Aegis-Proxy/main';
...
const UPDATE_REPO = 'fiavo/Aegis-Proxy';
async function runAutoUpdate(env, ns, ctx) {
	try {
		...
		try {
const res = await fetch(`https://raw.githubusercontent.com/${repo}/main/version.json`, { headers: { 'User-Agent': 'AegisProxy' }, cf: { cacheTtl: 0 } });
			if (res.ok) { const j = await res.json(); remoteVer = j && j.version ? String(j.version).replace(/^[vV]/, '') : null; }
		} catch (e) {}
		if (!remoteVer) {
			try {
				const res = await fetch(`https://raw.githubusercontent.com/${repo}/main/worker.js`, { headers: { 'User-Agent': 'AegisProxy' } });
		...
		if (!workerCode) {
			...
			const r = await fetch(codeUrl, { headers: { 'User-Agent': 'AegisProxy' } });
...
async function kabelGirsatNova() {
	for (const u of [urlGirsatNOVA, novaMachsanRaw + '/version.json']) {
		try { const r = await fetch(u, { headers: { 'User-Agent': 'AegisProxy' }, cf: { cacheTtl: 0 } }); if (r.ok) { const j = await r.json(); if (j && j.version) return j; } } catch (e) {}
	}
	return null;
}
...
function panelUnavailableHtml() {
	return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Aegis Proxy — setup</title>'
...
async function sherutMerkazMishtamesh(objMishtameshMinuy, env) {
	try {
		...
		const r = await fetch(base + '/user/index.html', { headers: { 'User-Agent': 'AegisProxy' }, cf: { cacheTtl: 300, cacheEverything: true } });
...
async function hasagatKidometNat64() {
	...
	if (/^https?:\/\//i.test(src)) {
		...
		try {
			const r = await fetch(src, { headers: { 'User-Agent': 'AegisProxy' } }); const txt = await r.text();
...
export default {
	async fetch(request, env, ctx) { try {
		...
		try {
			if (mitmonHagdarotReshet && (Date.now() - zmanMitmonHagdarotReshet) < 30000) {
				...
				hagdarotReshet = savedNS ? JSON.parse(savedNS) : {
					...
					autoUpdate: false, autoUpdateFormat: 'normal', autoUpdateInterval: 3600000, githubRepo: 'fiavo/Aegis-Proxy',
					telegramChannel: 'https://t.me/aegis_proxy',
					fakeConfigs: [
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (nativGisha === 'admin/keepalive') {
					...
					return new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8', 'Cache-Control': 'no-store', 'Set-Cookie': `auth=${_freshTok}; Path=/; HttpOnly; Secure; SameSite=Lax` } });
						...
						const label = encodeURIComponent('Aegis Proxy (' + url.host + ')');
						const otpauth = `otpauth://totp/${label}?secret=${secret}&issuer=${encodeURIComponent('Aegis Proxy')}&algorithm=SHA1&digits=6&period=30`;
				...
				if (nativGisha === 'admin/init') {// Reset config to defaults
						...
						if (nativGisha === 'admin/config.json') { // Save config.json
							...
							disguise: typeof settings.disguise === 'boolean' ? settings.disguise : false,
								...
								telegramChannel: String(settings.telegramChannel || '').trim().slice(0, 200) || 'https://t.me/aegis_proxy',
								...
								autoUpdate: typeof settings.autoUpdate === 'boolean' ? settings.autoUpdate : false,
									...
									githubRepo: String(settings.githubRepo || '').trim().slice(0, 100) || 'fiavo/Aegis-Proxy'
								};
						...
						if (request.method === 'POST') {
							...
							try {
								if (reqBody.fromCentral) {
									...
									let keys = []; try { const cr = await fetch(api + '/api/warp', { headers: { 'User-Agent': 'AegisProxy' } }); const cj = await cr.json(); keys = Array.isArray(cj.keys) ? cj.keys : []; } catch (e) {}
						...
						let textScript = '';
						try { const r = await fetch(ktovetKodMakor, { headers: { 'User-Agent': 'AegisProxy' } }); if (!r.ok) throw new Error('HTTP ' + r.status); textScript = await r.text(); } catch (e) { return _idkunTguvatShgia('Failed to download Worker source', { detail: (e && e.message) || String(e) }); }
						...
						try {
							const parsed = new URL(cleanUrl);
							const r = await fetch(`${parsed.protocol}//${parsed.host}/version`, { headers: { 'User-Agent': 'AegisProxy' }, signal: AbortSignal.timeout(12000) });
						...
						if (request.method === 'POST') {
							...
							if (body.action === 'check') {
								const repo = UPDATE_REPO;
								let remoteVer = null;
								try { const res = await fetch(`https://raw.githubusercontent.com/${repo}/main/version.json`, { headers: { 'User-Agent': 'AegisProxy' }, cf: { cacheTtl: 0 } }); if (res.ok) { const j = await res.json(); if (j && j.version) remoteVer = String(j.version).replace(/^[vV]/, ''); } } catch (e) {}
								const current = String(Version).replace(/^[vV]/, '');
					...
					subUserAgent: '', enableDirectConfigs: false, customRouting: '', metricNode: 'time.is'
						...
						const _pxSrc = {
							proxyip: 'https://raw.githubusercontent.com/fiavo/Aegis-Proxy/refs/heads/main/all.json',
						...
						try {
							const _pxRes = await fetch(_pxUrl, { headers: { 'User-Agent': 'AegisProxy' }, cf: { cacheTtl: 300, cacheEverything: true } });
				...
				const _subParamIsGenerator = url.searchParams.has('sub') && !imutViaSubKey;
					...
					if (bakashatMinuyLakoach || bakashatMinuyBackendHamara || keGenNivchar) {
						...
						try {
							...
							const _profileName = (config_JSON.muvcharMinuyMecholel.SUBNAME && config_JSON.muvcharMinuyMecholel.SUBNAME !== 'Aegis Proxy')
				...
				if (nativGisha === 'login' || nativGisha === 'admin' || nativGisha.startsWith('admin/')) {
					return new Response(
						'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Aegis Proxy — KV binding missing</title>'
						+ '<style>body{font-family:system-ui,Segoe UI,Tahoma,sans-serif;background:#0b0d11;color:#e9edf4;margin:0;display:flex;min-height:100vh;align-items:center;justify-content:center;padding:24px}'
						+ '.c{max-width:560px;background:#101319;border:1px solid #1c2027;border-radius:16px;padding:28px}h1{font-size:18px;margin:0 0 12px}p{color:#aeb6c4;line-height:1.7;font-size:14px}code{background:#0b0d11;border:1px solid #1c2027;border-radius:5px;padding:1px 6px;color:#22d3ee}</style></head>'
		...
		if (nativGisha === 'login' || nativGisha === 'admin' || nativGisha.startsWith('admin/')) {
			return new Response(
				'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Aegis Proxy — route not resolved</title>'
				+ '<style>body{font-family:system-ui,Segoe UI,Tahoma,sans-serif;background:#0b0d11;color:#e9edf4;margin:0;display:flex;min-height:100vh;align-items:center;justify-content:center;padding:24px}'
				+ '.c{max-width:560px;background:#101319;border:1px solid #1c2027;border-radius:16px;padding:28px}h1{font-size:18px;margin:0 0 12px}p{color:#aeb6c4;line-height:1.7;font-size:14px}code{background:#0b0d11;border:1px solid #1c2027;border-radius:5px;padding:1px 6px;color:#22d3ee}</style></head>'
		...
		if (_errPath === '/login' || _errPath === '/admin' || _errPath.startsWith('/admin/')) {
			const msg = (topLevelError && (topLevelError.stack || topLevelError.message)) || String(topLevelError);
			return new Response(
				'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Aegis Proxy — error</title>'
...
async function peimatLevMerkazit(env) {
	...
	try {
		await fetch(api + '/heartbeat', { method: 'POST', headers: { 'Content-Type': 'application/json', 'User-Agent': 'AegisProxy' }, body: JSON.stringify({ id, host, version: Version, monthTraffic: usage ? usage.total : 0, ts: Date.now() }) });
	} catch (e) { /* best-effort */ }
}
async function riaanunHodaot(env) {
	const { api } = await kabelCentralApi(env); if (!api) return;
	try {
		const r = await fetch(api + '/announcement', { headers: { 'User-Agent': 'AegisProxy' } });
...
async function hasagatKovetzBreicha(fileUrl) {
	...
	let list = [];
	try {
		const r = await fetch(fileUrl, { headers: { 'User-Agent': 'AegisProxy' }, cf: { cacheTtl: 1800, cacheEverything: true } });
...
async function runCfInstall(env, botToken, chatId, host, request) {
	const a = ['https://', 'raw.', 'githubusercontent', '.com/', 'fiavo/', 'Aegis-Proxy/', 'main/', 'version.json'].join('');
	const b = ['https://', 'raw.', 'githubusercontent', '.com/', 'fiavo/', 'Aegis-Proxy/', 'main/', 'worker.js'].join('');
